...
export function PasswordAuthSection({ accessMode }: { accessMode: 'lan' | 'all' }) {
  ...
  return (
    <div className="mt-4 p-3 rounded border" style={{ borderColor: 'color-mix(in srgb, var(--color-text-muted) 20%, transparent)' }}>
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>Password authentication</h3>
        <span className="text-xs" style={{ color: isSet ? 'var(--color-primary)' : 'var(--color-text-muted)' }}>
          {isSet ? 'Enabled' : 'Disabled'}
        </span>
      </div>

      {!isSet && accessMode === 'all' && (
        <div className="mb-3 p-2 text-xs rounded" style={{ background: 'color-mix(in srgb, var(--color-error) 15%, transparent)', color: 'var(--color-text)' }}>
      ...
      {isSet ? (
        <>
          <div className="text-xs mb-2" style={{ color: 'var(--color-text-muted)' }}>Password is set.</div>
          <div className="flex items-center gap-3 mb-2 text-xs" style={{ color: 'var(--color-text)' }}>
            <label className="flex items-center gap-2">Session (days)
              <input type="number" min={1} value={sessionDays} onChange={(e) => commitSessionDays(parseInt(e.target.value, 10) || 0)}
                ...
                style={{ borderColor: 'color-mix(in srgb, var(--color-text-muted) 30%, transparent)', backgroundColor: 'var(--color-bg)', color: 'var(--color-text)' }} />
            </label>
          ...
          <div className="flex gap-2">
            <button onClick={() => setModalOpen(true)} className="text-xs px-2 py-1 rounded border"
              style={{ borderColor: 'color-mix(in srgb, var(--color-text-muted) 30%, transparent)', color: 'var(--color-text)' }}>Change password</button>
            <button onClick={handleDisable} className="text-xs px-2 py-1 rounded border"
              style={{ borderColor: 'color-mix(in srgb, var(--color-error) 40%, transparent)', color: 'var(--color-error)' }}>Disable</button>
          </div>
        </>
      ) : (
        <button onClick={() => setModalOpen(true)} className="text-xs px-2 py-1 rounded font-medium"
          style={{ backgroundColor: 'var(--color-primary)', color: 'var(--color-text-contrast)' }}>Set password</button>
      )}
